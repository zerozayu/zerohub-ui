...
import { ElUpload, ElButton, TabPaneName } from "element-plus";
...
/**
 * 对每一个文件对象进行分片、校验是否已上传，把待上传的文件信息返回
 * @param file 文件对象本身
 */
// 将文件按固定大小进行切片(5MB)
const chunkSize = Number.parseInt(import.meta.env.VITE_APP_CHUNK_SIZE);
  ...
  fileList.forEach((fileInfo, index) => {
    ...
    chunkList.forEach((chunkInfo, index) => {
      const fn = () => {
        ...
        formData.append("chunkNo", chunkInfo.chunkNo.toString());
...
const sendMergeFileListRequest = (filename: string, fileHash: string) => {
  ...
  uploadHttp.merge(filename, fileHash, chunkSize).then((res) => {
    if (res.code === 200) {
